    	...
    	$html  = '';
       	$html .= '<div data-table-id="'.esc_attr($fields_type).'" class="row ppom-tabs ppom-fields-actions" data-field-no="'.esc_attr($field_index).'">';
			...
			$html .= '<div class="col-md-12 ppom-tabs-header">';
				...
				$html .= '<label for="tab1" id="tab1" class="ppom-tabs-label ppom-active-tab">'.esc_html__( 'Campos', 'ppom' ).'</label>';
	...
	function render_all_input_types($name, $data, $fields_type, $field_index, $values ) {
		...
		switch ($type) {
			...
			case 'textarea' :
				...
				$html_input .= '<textarea data-metatype="'.esc_attr($name).'" class="form-control ppom-meta-field"';
							...
							foreach ($values as $opt_index => $pre_uploaded_image){
								...
								$html_input .= '<li class="data-options">';
									...
									$html_input .= '<div style="display: flex;">';
										...
										$html_input .= '<input class="form-control" type="text" placeholder="Precio (Ejemplo: +15%)" value="'.esc_attr(stripslashes($pre_uploaded_image['price'])).'" name="ppom['.esc_attr($field_index).'][images]['.esc_attr($opt_index).'][price]">';
						...
						foreach ($values as $opt_index => $pre_uploaded_image){
							...
							$html_input .= '<li class="data-options">';
								...
								$html_input .= '<div style="display: flex;">';
									...
									$html_input .= '<input class="form-control" type="text" placeholder="Precio (Ejemplo: +15%)" value="'.esc_attr($media_price).'" name="ppom['.esc_attr($field_index).'][audio]['.esc_attr($opt_index).'][price]">';
									$html_input .= '<button class="btn btn-danger ppom-pre-upload-delete" style="height: 35px;"><i class="fa fa-times" aria-hidden="true"></i></button>';
								$html_input .= '</div>';
							$html_input .= '</li>';
