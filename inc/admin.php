...
function ppom_admin_update_form_meta()
{
	...
	if ($rows_effected) {
		...
		$resp = array(
			'message' => __('Formulario actualizado exitosamente', 'ppom'),
			'status' => 'success',
			'productmeta_id' => $productmeta_id
		);
	} else {
		...
		$resp = array(
			'message' => __('No se encontraron cambios, por favor, cambie e intente de nuevo.', 'ppom'),
			'status' => 'success',
			'productmeta_id' => $productmeta_id
		);
	}
...
function ppom_admin_delete_meta()
{
	...
	if ($res) {
		...
		_e('Grupo fue eliminado correctamente', 'ppom');
	} else {
		$wpdb->show_errors();
		$wpdb->print_error();
	}
...
function ppom_admin_simplify_meta($meta)
{
	...
	if ($metas) {
		...
		foreach ($metas as $meta => $data) {
			...
			echo '<li>';
			echo '<strong>etiqueta:</strong> ' . $title;
			echo ' | <strong>tipo:</strong> ' . $type;

			if (!is_object($options) && is_array($options)) {
				echo ' | <strong>opciones:</strong> ';
			...
			echo ' | <strong>necesario:</strong> ' . $req;
			echo '</li>';
		}
